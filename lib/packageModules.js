...
function zip(directory, name) {
  ...
  const artifactFilePath = path.join(this.serverless.config.servicePath, '.webpack', name);
...
module.exports = {
  packageModules() {
    const funcs = this.entryFunctions || [this.serverless.service.getServiceObject().name];

    return BbPromise.mapSeries(funcs, (entryFunction, index) => {
      const filename = `${entryFunction.funcName || this.serverless.service.getServiceObject().name}.zip`;

      if (this.skipCompile) {
        // Copy the from .webpack to .serverless
        const artifactSource = path.join(this.serverless.config.servicePath, '.webpack', filename);
        const artifactDestination = path.join(this.serverless.config.servicePath, '.serverless', filename);

        console.log(`Copying existing artifact from ${artifactSource} to ${artifactDestination}`);
        ...
        if (_.get(this.serverless, 'service.package.individually')) {
          setArtifactPath.call(
            ...
            entryFunction.func,
            path.relative(this.serverless.config.servicePath, artifactDestination)
          );
        }

        return Promise.resolve(artifactDestination);
      }

      const stats = _.get(this.compileStats.stats, index, {});

      const modulePath = stats.compilation.compiler.outputPath;
